...
static void simulate_tournament(void)
{
    ...
    ChampionshipRecord record = buildChampionshipPath(&bracket, champion);
    printChampionshipPath(&record);
    printf("\n");

    printKnResults(&bracket);

    
    freeMatchData(&record);

    /* --- Exit/Utility Module: restart-or-exit prompt --- */
    int restart = promptUserRestart();

    if (restart) {
        restartSim(g_ctx->teams, g_ctx->teamCount, &bracket);
        printf("\nSimulation data reset. Choose \"Simulate a Tournament\" again to run a new bracket.\n");
    } else {
        /* exitSim() calls exit() and never returns, so it bypasses the
         * rest of main()'s loop and cleanup_system() -- close the log
         * file properly here first, the one piece of teardown exitSim
         * doesn't know about (it only owns Team/Bracket/ChampionshipRecord
         * memory, not the SystemContext's log file). */
        if (g_ctx->logFile != NULL) {
            fprintf(g_ctx->logFile, "=== Session ended ===\n");
            fclose(g_ctx->logFile);
            g_ctx->logFile = NULL;
        }
        exitSim(g_ctx->teams, &bracket, &record); /* never returns */
    }
}
